...
use scallop_core::runtime::dynamic::CommunicationContext;
use scallop_core::runtime::env::*;
use scallop_core::runtime::monitor;
use scallop_core::runtime::provenance::*;
use scallop_core::utils::*;
use scallop_core::runtime::dynamic::RuntimeChannel;
...
impl Context {
  ...
  fn compile_with_output_relations(&mut self, outputs: Vec<&str>) -> Result<(), BindingError> {
    match_context!(&mut self.ctx, c, c.compile_with_output_relations(Some(outputs)).map_err(BindingError::from))
  }
  ...
  <C as Provenance>::Tag: std::marker::Sync + std::marker::Send,
{
  ...
  let communication_contexts = {
    let mut communication_contexts: VecDeque<CommunicationContext<C>> = VecDeque::new();
    let mut leader_txs = Vec::new();
    let mut leader_rxs = Vec::new();

    // create synchronization objects for all `batch_size-1` followers
    for i in 1..batch_size {
      let (init_tx, init_rx) = std::sync::mpsc::channel();
      let (respond_tx, respond_rx) = std::sync::mpsc::channel();
      let synchronization = CommunicationContext {
        id: i,
        thread_count: batch_size,
        channel: RuntimeChannel::Follower(init_tx, respond_rx),
      };
      leader_rxs.push(init_rx);
      leader_txs.push(respond_tx);
      communication_contexts.push_back(synchronization);
    }
    // create the synchronization object for the leader
    let leader_sync = CommunicationContext {
      id: 0,
      thread_count: batch_size,
      channel: RuntimeChannel::Leader(leader_rxs, leader_txs),
    };
    communication_contexts.push_front(leader_sync);
    communication_contexts
  };

  communication_contexts.into_par_iter().enumerate()
    .map(|(i, communication_context)| {
      let output_relations = &batch_output_relations[i];
      let mut temp_ctx = internal.clone();
      temp_ctx.exec_ctx.communication = Some(communication_context);
