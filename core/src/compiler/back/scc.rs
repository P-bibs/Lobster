...
impl DependencyGraph {
  ...
  pub fn stratify(&self) -> Result<Vec<Stratum>, SCCError> {
    ...
    let mut source_nodes = scc_graph
      .node_indices()
      .filter(|n| scc_graph.edges(*n).next().is_none())
      .collect::<Vec<_>>();
    source_nodes.sort_by(|a, b| scc_graph[*a].cmp(&scc_graph[*b]));
    ...
    fringe.make_contiguous().sort_by(|a, b| scc_graph[*a].cmp(&scc_graph[*b]));
    ...
    if let Ok(reordering) = std::env::var("REORDER") {
      let reorders = reordering.split(",").map(|s| -> (i32,i32) {
          let l = s.split("<").collect::<Vec<_>>();
          assert_eq!(l.len(), 2, "Reorder string should be in the form of `s<n`");
          (l[0].parse().unwrap(), l[1].parse().unwrap())
        }).collect::<HashMap<i32,i32>>();
      for (&i, &move_back_amount) in reorders.iter() {
        let strata = stratas.remove(i as usize);
        let new_index = (i - move_back_amount) as usize;
        stratas.insert(new_index, strata);
      }
    };
