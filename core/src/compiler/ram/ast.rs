...
impl Stratum {
  ...
  pub fn source_relations(&self) -> HashSet<String> {
    self
      .updates
      .iter()
      .flat_map(|update| update.dataflow.source_relations())
      .cloned()
      .collect()
  }
}
...
pub enum Dataflow {
  ...
  Join(Box<Dataflow>, Box<Dataflow>, bool),
...
impl Dataflow {
  ...
  pub fn join(self, d2: Dataflow) -> Self {
    Self::Join(Box::new(self), Box::new(d2), false)
  }
  ...
  pub fn find_subtree_mut<'a, 'b>(&'a mut self, target: &'b Self) -> Option<&'a mut Self> {
    if self == target {
      Some(self)
    } else {
      match self {
        ...
        | Self::Difference(d1, d2) => {
          if let Some(subtree) = d1.find_subtree_mut(target) {
            return Some(subtree);
          }
          d2.find_subtree_mut(target)
        }
        Self::Project(d, _)
        | Self::Filter(d, _)
        | Self::Find(d, _)
        ...
        | Self::ForeignPredicateJoin(d, _, _)
        | Self::Sorted(d)
        | Self::JoinIndexedVec(d, _)
        | Self::Exclusion(d, _) => d.find_subtree_mut(target),
        Self::Reduce(_) => None,
        Self::Relation(_) => None,
        Self::ForeignPredicateGround(_, _) | Self::UntaggedVec(_) => None,
      }
    }
  }

  pub fn find_subtree_all<'a, 'b>(&'a self, target: &'b Self) -> Vec<&'a Self> {
    let mut in_children = match self {
      Self::Unit(_) => Vec::new(),
      Self::Union(d1, d2)
      | Self::Join(d1, d2, _)
      | Self::Intersect(d1, d2)
      ...
      | Self::Antijoin(d1, d2)
      | Self::Difference(d1, d2) => {
        let mut in_left = d1.find_subtree_all(target);
        in_left.extend(d2.find_subtree_all(target));
        in_left
      }
      Self::Project(d, _)
      | Self::Filter(d, _)
      ...
      | Self::Sorted(d)
      | Self::JoinIndexedVec(d, _)
      | Self::Exclusion(d, _) => d.find_subtree_all(target),
      Self::Reduce(_) => Vec::new(),
      Self::Relation(_) => Vec::new(),
      Self::ForeignPredicateGround(_, _) | Self::UntaggedVec(_) => Vec::new(),
    };
    if self == target {
      in_children.push(self);
    }
    in_children
  }
  ...
  pub fn substitute_relation(&mut self, source: &str, target: &str) -> bool {
    ...
    match self {
      ...
      Self::ForeignPredicateGround(_, _) | Self::UntaggedVec(_) => false,
    }
  }
  pub fn result_type(&self, sources: &HashMap<String, TupleType>) -> TupleType {
    match self {
      // Cases that require some actual work
      Self::Relation(r) => sources[r].clone(),
      Self::Project(d, expr) => {
        let t = d.result_type(sources);
        expr.result_type(&t)
      }
      Self::Join(d1, d2, _) => {
        let t1 = d1.result_type(sources);
        let t2 = d2.result_type(sources);
        t1.append(&t2.pop_first())
      }
      Self::Product(d1, d2) => {
        let t1 = d1.result_type(sources);
        let t2 = d2.result_type(sources);
      ...
      | Self::Union(d, _)
      | Self::Difference(d, _)
      | Self::OverwriteOne(d) => d.result_type(sources),

      // Not sure of the semantics of these operators
      Self::Unit(_) | Self::UntaggedVec(_) | Self::Antijoin(_, _) | Self::Reduce(_) | Self::Exclusion(_, _) => {
        unimplemented!()
      }

      // Don't need
      Self::ForeignPredicateGround(_, _)
      | Self::ForeignPredicateConstraint(_, _, _)
      | Self::ForeignPredicateJoin(_, _, _) => unimplemented!(),
      e => panic!("Cannot determine result type of {:?}", e),
    }
  }
}
