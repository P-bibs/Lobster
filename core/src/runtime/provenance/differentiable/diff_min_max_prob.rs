...
use crate::utils::*;
use crate::gpu_runtime::tag::C_Prob;
...
impl<T: FromTensor, P: PointerFamily> Provenance for DiffMinMaxProbProvenance<T, P> {
  ...
  fn to_ffi_tag(tag: &Self::Tag) -> Self::FFITag {
    C_Prob {
      prob: tag.0,
      deriv: match tag.1 {
        Derivative::Pos(i) => i as isize,
        Derivative::Zero => 0,
        Derivative::Neg(i) => -(i as isize),
      },
    }
  }
  fn from_ffi_tag(tag: &Self::FFITag) -> Self::Tag {
    Prob(tag.prob, match tag.deriv {
      i if i > 0 => Derivative::Pos(i as usize),
      0 => Derivative::Zero,
      i => Derivative::Neg(-i as usize),
    })
  }
