...
use crate::gpu_runtime::{array::C_Array, tag::{C_DualNumber, C_Gradient}};
...
impl<T: FromTensor, P: PointerFamily> Provenance for DiffAddMultProbProvenance<T, P> {
  ...
  type FFITag = C_DualNumber;

  fn to_ffi_tag(tag: &Self::Tag) -> Self::FFITag {
    C_DualNumber {
      real: tag.real,
      gradient: C_Gradient {
        indices: C_Array::new(tag.gradient.indices.clone()),
        values: C_Array::new(tag.gradient.values.clone()),
      },
    }
  }
  fn from_ffi_tag(tag: &Self::FFITag) -> Self::Tag {
    DualNumber2 {
      real: tag.real,
      gradient: Gradient {
        indices: tag.gradient.indices.clone_to_vec(),
        values: tag.gradient.values.clone_to_vec(),
      },
    }
  }
