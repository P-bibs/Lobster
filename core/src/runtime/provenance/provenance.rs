...
use crate::gpu_runtime::tag::C_Provenance;

use super::*;

#[derive(Clone, Debug)]
pub struct DefaultFFITag;
impl Display for DefaultFFITag {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "DefaultFFITag")
  }
}

/// A provenance
pub trait Provenance: Clone + 'static {
  fn print_storage(&self) {unreachable!();}
  ...
  type FFITag: Clone + Debug + Display = DefaultFFITag;
  fn to_ffi_tag(_tag: &Self::Tag) -> Self::FFITag {
    todo!();
  }
  fn from_ffi_tag(_tag: &Self::FFITag) -> Self::Tag {
    todo!();
  }
  fn to_ffi_provenance(ctxes: &Vec<Self>) -> C_Provenance {
    panic!("libsclgpu does not support {}", ctxes[0].name());
  }
