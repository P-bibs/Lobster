...
impl<Prov: Provenance> std::cmp::PartialEq for DynamicCollection<Prov> {
  fn eq(&self, other: &Self) -> bool {
    match (self, other) {
      (Self::Sorted(s1), Self::Sorted(s2)) => s1.elements == s2.elements,
      (Self::IndexedVec(_), Self::IndexedVec(_)) => unimplemented!(),
      (Self::DenseMatrix(_), Self::DenseMatrix(_)) => unimplemented!(),
      _ => false,
    }
  }
}
...
impl<Prov: Provenance> DynamicCollection<Prov> {
  pub fn elements(&self) -> &Vec<DynamicElement<Prov>> {
    match self {
      Self::Sorted(s) => &s.elements,
      Self::IndexedVec(_) => unimplemented!(),
      Self::DenseMatrix(_) => unimplemented!(),
    }
  }
  pub fn from_vec(elements: Vec<DynamicElement<Prov>>, ctx: &Prov) -> Self {
    DynamicCollection::Sorted(DynamicSortedCollection::from_vec(elements, ctx))
  }
...
impl<'a, Prov: Provenance> std::cmp::PartialEq for DynamicCollectionRef<'a, Prov> {
  fn eq(&self, other: &Self) -> bool {
    match (self, other) {
      (Self::Sorted(s1), Self::Sorted(s2)) => s1.elements == s2.elements,
      (Self::IndexedVec(_), Self::IndexedVec(_)) => unimplemented!(),
