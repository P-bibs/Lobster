...
/// The trait for a batch of dynamic elements
pub trait Batch<'a, Prov: Provenance>: 'a + dyn_clone::DynClone {
  /// Get the next element in the batch
  ///
  /// This function needs to be implemented by concrete types that is a `Batch``
  fn next_elem(&mut self) -> Option<DynamicElement<Prov>>;

  /// Take n elements from the batch; if the batch does not contain enough
  /// elements, take the whole batch
  fn take_n_elems(&mut self, n: usize) -> Vec<DynamicElement<Prov>> {
  ...
  /// Take all the elements from the batch
  fn take_all_elems(&mut self) -> Vec<DynamicElement<Prov>> {
    let mut result = Vec::new();
    while let Some(elem) = self.next_elem() {
      result.push(elem);
    }
