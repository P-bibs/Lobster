...
pub struct ExtensionalRelation<Prov: Provenance> {
  ...
  pub program_facts: Vec<(DynamicInputTag, Tuple)>,
...
impl<Prov: Provenance> std::fmt::Display for ExtensionalRelation<Prov> {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    write!(f, "Program Facts: {:?}\n", self.program_facts)?;
    write!(f, "Internalize Program Facts: {:?}\n", self.internalized_program_facts)?;
    write!(f, "Loaded Files: {:?}\n", self.loaded_files)?;
    write!(f, "Dynamic Input: {:?}\n", self.dynamic_input)?;
    write!(f, "Static Input: {:?}\n", self.static_input)?;
    write!(f, "Internal: {:?}\n", self.internal)?;
    write!(f, "Internalized: {:?}\n", self.internalized)
  }
}
