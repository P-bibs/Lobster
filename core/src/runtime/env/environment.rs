...
impl RuntimeEnvironment {
  ...
  pub fn eval_binary(&self, expr: &BinaryExpr, v: &Tuple) -> Option<Tuple> {
    ...
    let result = match (&expr.op, lhs_v, rhs_v) {
      ...
      (Mul, Tuple::Value(F32(i1)), Tuple::Value(F32(i2))) => {
        let r = i1 * i2;
        if r.is_nan() {
          return None;
        } else {
      ...
      (Mul, Tuple::Value(F64(i1)), Tuple::Value(F64(i2))) => {
        let r = i1 * i2;
        if r.is_nan() {
          return None;
        } else {
    ...
    match (&expr.op, arg_v) {
      ...
      (TypeCast(dst), arg) => {
        ...
        match (arg, dst) {
          ...
          (Tuple::Value(I8(i)), T::F32) => Some(Tuple::Value(F32(OrderedFloat(i as f32)))),
          ...
          (Tuple::Value(I32(i)), T::F32) => Some(Tuple::Value(F32(OrderedFloat(i as f32)))),
          ...
          (Tuple::Value(USize(i)), T::F32) => Some(Tuple::Value(F32(OrderedFloat(i as f32)))),
          (Tuple::Value(USize(i)), T::F64) => Some(Tuple::Value(F64(i as f64))),

          (Tuple::Value(U32(i)), T::USize) => Some(Tuple::Value(USize(i as usize))),
          ...
          (Tuple::Value(F64(f)), T::F32) => Some(Tuple::Value(F32(OrderedFloat(f as f32)))),
