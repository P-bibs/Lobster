...
pub trait ForeignPredicate: DynClone {
  ...
  /// Whether the foreign predicate is batched; default to be batched
  fn batched(&self) -> bool {
    false
  }

  /// The maximum batch size; if None, then there is no maximum batch size
  ///
  /// This function will not be used if the foreign predicate is not batched
  ...
  /// Evaluate the foreign predicate given a batch of tuples containing bounded variables
  ///
  /// Default implementation is that it will call `evaluate_with_env` on every single input in the batch
  #[allow(unused_variables)]
  fn evaluate_batch_with_env(&self, env: &RuntimeEnvironment, batched_input: Vec<&[Value]>) -> Vec<Vec<(DynamicInputTag, Vec<Value>)>> {
    batched_input
      .into_iter()
      .map(|input| self.evaluate_with_env(env, input))
      .collect()
  }
}
...
impl ForeignPredicate for DynamicForeignPredicate {
  ...
  fn evaluate_batch_with_env(&self, env: &RuntimeEnvironment, batched_input: Vec<&[Value]>) -> Vec<Vec<(DynamicInputTag, Vec<Value>)>> {
    self.fp.evaluate_batch_with_env(env, batched_input)
  }
}
