...
impl TupleType {
  pub fn append(&self, other: &Self) -> Self {
    match (self, other) {
      (Self::Tuple(ts), Self::Tuple(os)) => {
        let mut new_ts = ts.clone().to_vec();
        new_ts.extend_from_slice(os);
        Self::Tuple(new_ts.into_boxed_slice())
      }
      _ => panic!("Can only append tuples to tuples"),
    }
  }
    ...
    match self {
      Self::Tuple(ts) => {
        if ts.is_empty() {
          ...
          Self::Tuple(ts[1..].to_vec().into_boxed_slice())
        }
      }
      _ => panic!("Cannot pop first element of Value type"),
    }
  }
}
