    ...
    match &entity {
      ast::Entity::Expr(e) => match e {
        ast::Expr::Constant(c) => match (&c, ty) {
          ...
          (ast::Constant::Integer(i), ValueType::F32) => Ok(Value::F32(OrderedFloat(i.int().clone() as f32))),
          (ast::Constant::Integer(i), ValueType::F64) => Ok(Value::F64(i.int().clone() as f64)),
          (ast::Constant::Float(f), ValueType::F32) => Ok(Value::F32(OrderedFloat(f.float().clone() as f32))),
