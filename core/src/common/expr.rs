...
impl Expr {
  ...
  pub fn result_type(&self, source_type: &TupleType) -> TupleType {
    match self {
      Self::Tuple(t) => TupleType::Tuple(t.iter().map(|e| e.result_type(source_type)).collect()),
      Self::Access(a) => source_type.index(a).clone(),
      Self::Constant(c) => TupleType::Value(c.value_type()),
      Self::Binary(b) => {
        let t1 = b.op1.result_type(source_type);
        let t2 = b.op2.result_type(source_type);
        match b.op {
          BinaryOp::Mod | BinaryOp::Add | BinaryOp::Sub | BinaryOp::Mul | BinaryOp::Div => {
            if t1 != t2 {
              panic!("Type mismatch")
            } else {
              t1
          ...
          BinaryOp::And | BinaryOp::Or | BinaryOp::Xor => {
            TupleType::Value(ValueType::Bool)
          },
      ...
      Self::Unary(u) => {
        let t = u.op1.result_type(source_type);
        match u.op {
          UnaryOp::Not => {
            if t == TupleType::Value(ValueType::Bool) {
              TupleType::Value(ValueType::Bool)
            } else {
              panic!("Type mismatch")
            }
      ...
      Self::IfThenElse(i) => {
        let t1 = i.then_br.result_type(source_type);
        let t2 = i.else_br.result_type(source_type);
        if t1 == t2 {
          t1
        } else {
          panic!("Type mismatch")
      ...
      o => panic!("Operator {:?} not supported", o),
    }
  }
}
