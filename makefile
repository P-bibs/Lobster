...
install-scli:
	cd core/src/gpu_runtime/libsclgpu/cmake-release && /cmake --build . -t sclgpu -j
...
build-scli-dev:
	cd core/src/gpu_runtime/libsclgpu/cmake-debug && cmake --build . -t sclgpu -j
	cargo build --bin scli
...
develop-scallopy:
	cd core/src/gpu_runtime/libsclgpu/cmake-debug && cmake --build . -t sclgpu -j
	cd etc/scallopy; maturin develop
develop-scallopy-release:
	cd core/src/gpu_runtime/libsclgpu/cmake-release && cmake --build . -t sclgpu -j80
...
init-venv:
	...
	.env/bin/pip install maturin torch torchvision transformers gym scikit-learn opencv-python tqdm matplotlib wandb
