  ...
  def __init__(self, size):
    ...
    self.scl = scallopy.Module(
      program="""
        ...
        type perm(x: u64, y: u64)
        type filtered(s: u64)

        rel perm(x,y) = digit1(x,y)
        rel filtered(s) = perm(s,s)
      """,
      provenance="diffminmaxprob",
      k=1,
      input_mappings={
        "digit1": [(i, j) for i in range(size) for j in range(size)],
        },
      output_mappings={"filtered": list(range(size))},
