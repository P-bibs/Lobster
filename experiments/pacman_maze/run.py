...
// Basic connectivity
type edge(x: u32, y: u32, xp: u32, yp: u32, a: u32)
...
rel next_action_(a) = next_position(a, x, y), goal(gx, gy), path(x, y, gx, gy)
rel next_action(a) = next_action_(a)

// Constraint violation
rel too_many_goal() = n := count(x, y: goal(x, y)), n > 1
rel too_many_enemy() = n := count(x, y: enemy(x, y)), n > {num_enemies}
...
rel edge_(ax * {grid_width} + ay, bx * {grid_width} + by, a) = edge(ax, ay, bx, by, a)
rel node_(x * {grid_width} + y) = node(x, y)

// Get the next position
rel next_position(a, xp, yp) = actor(x, y), edge(x, y, xp, yp, a)
rel next_position_(a, x * {grid_width} + y) = next_position(a, x, y)

rel goal_(x * {grid_width} + y) = goal(x, y)

// Path for connectivity; will condition on no enemy on the path
rel path_(p, p) = node_(p)
rel path_(p1, p2) = edge_(p1, p2, _)
rel path_(p1, p2) = path_(p1, pm), edge_(pm, p2, _)

rel next_action_(a) = next_position_(a, p), goal_(pg), path_(p, pg)
rel next_action(a) = next_action_(a)
...
class PolicyNet(nn.Module):
  ...
  def __init__(self, grid_x, grid_y, cell_pixel_size):
    ...
    self.path_planner = scallopy.Module(
      program=program_1d.format(grid_width=self.grid_x, num_enemies=args.num_enemies),
  ...
  def forward(self, x):
    ...
    self.neural_time = time.time() - now
    now = time.time()
    result = self.path_planner(actor=actor, goal=goal, enemy=enemy, outputs = ["next_action", "violation"])
    if False:
      if False:
        edge = result[0]
        edges = ('\n'.join([f"e{x1}_{y1} -> e{x2}_{y2};" for (x1, y1, x2, y2, dir) in edge]))
        nodes = ('\n'.join([f"e{x}_{y} [pos=\"{x},{y}!\"];" for x in range(5) for y in range(5)]))
        print(edges)
        print(nodes)
      else:
        edge = result[0]
        edges = ('\n'.join([f"e{p1} -> e{p2};" for (p1, p2, dir) in edge]))
        nodes = ('\n'.join([f"e{x * 5 + y} [pos=\"{x},{y}!\"];" for x in range(5) for y in range(5)]))
        print(edges)
        print(nodes)
      import code; code.interact(local=locals())
    self.scallop_time = time.time() - now
...
class DQN:
  ...
  def optimize_model(self):
    ...
    back_time = time.time() - now
    for param in self.policy_net.parameters():
      param.grad.data.clamp_(-1, 1)
    self.optimizer.step()

    #print(self.policy_net.neural_time,
    #      self.policy_net.scallop_time,
    #      self.policy_net.path_planner.forward_fn.scallop_back_time,
    #      back_time - self.policy_net.path_planner.forward_fn.scallop_back_time,
    #      sep=",",
    #      file=sys.stderr
    #      )
...
class Trainer:
  def __init__(self, grid_x, grid_y, cell_size, dpi, num_enemies, epsilon, num_epochs, args):
    ...
    if (self.args.load_best):
      self.dqn.policy_net.load_state_dict(torch.load(os.path.join(FILE_DIR, "checkpoints", "best_policy_net.pth")))
      self.dqn.target_net.load_state_dict(torch.load(os.path.join(FILE_DIR, "checkpoints", "best_target_net.pth")))

    self.epsilon = epsilon
    self.best_success_rate = 0

  def show_image(self, raw_image, torch_image):
    if self.args.overlay_prediction:
      self.dqn.policy_net.visualize(self.arena, raw_image, torch_image)
    cv2.namedWindow("Current Arena", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Current Arena", self.args.window_size, self.args.window_size)
    cv2.imshow("Current Arena", raw_image)
    cv2.waitKey(int(self.args.show_run_interval * 1000))
  ...
  def train_epoch(self, epoch):
    ...
    for episode_i in iterator:
      print("=====Episode", episode_i, flush=True)
      _ = self.arena.reset()
  ...
  def test_epoch(self, epoch):
    ...
    if success_rate > self.best_success_rate:
      ...
      torch.save(self.dqn.policy_net.state_dict(), os.path.join(FILE_DIR, "checkpoints", "best_policy_net.pth"))
      torch.save(self.dqn.target_net.state_dict(), os.path.join(FILE_DIR, "checkpoints", "best_target_net.pth"))
...
if __name__ == "__main__":
  ...
  print("Switching over to doubled grid size")
  args.grid_x *= args.multiplier
  args.grid_y *= args.multiplier
  args.num_epochs -= args.switchover

  if args.lobster:
    import os
    os.environ["STRATUM"]="0,14"
    os.environ["USE_ARENA"] = "1"
    os.environ["ARENA_SIZE"] = "3"
    os.environ["SUPSPACE"] = "3"
    os.environ["CUB"] = "1"
