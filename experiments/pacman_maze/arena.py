...
class AvoidingArena(gym.Env):
  def __init__(
    ...
    if self.background_image.shape[0] < h or self.background_image.shape[1] < w:
      # resize using tiling
      self.background_image = cv2.repeat(self.background_image, int(h / self.background_image.shape[0]) + 1, int(w / self.background_image.shape[1]) + 1)
