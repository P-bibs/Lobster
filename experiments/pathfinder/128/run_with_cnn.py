...
def pathfinder_128_loader(data_root, batch_size, train_percentage, difficulty):
...
class Trainer():
  def __init__(self, train_loader, test_loader, device, model_root, learning_rate, use_chain, provenance, k, grid_x, grid_y):
    self.device = device
    self.network = PathFinder128Net(provenance, k, use_chain=use_chain, num_block_x=grid_x, num_block_y=grid_y).to(device)
  ...
  def train(self, epoch):
    ...
    for (input, _, expected_output) in iter:
      ...
      #print(self.network.neural_time,
      #      self.network.scallop_time,
      #      self.network.connected.scallop_back_time,
      #      back_time - self.network.connected.scallop_back_time,
      #      sep=",",
      #      file=sys.stderr
      #      )
...
if __name__ == "__main__":
  ...
  # Load data
  (train_loader, test_loader) = pathfinder_128_loader(data_root, args.batch_size, args.train_percentage, args.difficulty)

  wandb.init(project="pathfinder_128", config=vars(args))
