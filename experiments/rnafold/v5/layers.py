...
class TransformerStructurePredictor(nn.Module):
    def __init__(self, dropout_rate=0.0, embedding_size=64, nhead=5):
        self.dropout = nn.Dropout(p=dropout_rate)
        self.embedding = SparseEmbedding(dim=embedding_size)
        self.transformer = nn.TransformerEncoderLayer(embedding_size, nhead=nhead)
...
class PairedLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        B, N, _, C = x.shape
        x = x.permute(0, 3, 1, 2)
        x_u = torch.triu(x.reshape(B * C, N, N), diagonal=diag).view(B, C, N, N)
        x_l = torch.tril(x.reshape(B * C, N, N), diagonal=-1).view(B, C, N, N)
...
class NeuralNet(nn.Module):
    def __init__(self, embed_size=0, is_hyena = 0,
    ...
    def forward(self, seq):
        ...
        if self.is_hyena == 1:
          x = self.embedding(seq)[0].to(device) # (B, 4, N)
        else:
          x = self.embedding(['0' + s for s in seq]).to(device)
