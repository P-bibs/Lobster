...
class HWFDataset(torch.utils.data.Dataset):
  def __init__(self, root: str, dataset: str):
    ...
    self.root = root
    self.metadata = json.load(open(os.path.join(root, f"HWF/{dataset}")))
  ...
  def __getitem__(self, index):
    ...
    return (img_seq, img_seq_len, res, formula)
  ...
  def collate_fn(batch):
    ...
    formulas = [formula for (_, _, _, formula) in batch]
    return (img_seqs, img_seq_len, results, formulas)


def hwf_loader(data_dir, train_dataset, test_dataset, batch_size):
  train_loader = torch.utils.data.DataLoader(HWFDataset(data_dir, train_dataset), collate_fn=HWFDataset.collate_fn, batch_size=batch_size, shuffle=False)
  test_loader = torch.utils.data.DataLoader(HWFDataset(data_dir, test_dataset), collate_fn=HWFDataset.collate_fn, batch_size=batch_size, shuffle=False)
...
class HWFNet(nn.Module):
  def __init__(self, no_sample_k, sample_k, provenance, k, debug=False):
    ...
    if self.debug:
      ...
      self.eval_formula = self.ctx.forward_function("result", dispatch="single", jit=args.jit, recompile=args.recompile)
  ...
  def _forward_with_sampling(self, batch_size, formula_length, img_seq, img_seq_len, length):
    ...
    #print("symbol_facts")
    #for x in symbol_facts:
    #  print(x)
    #print("",flush=True)
    #ops = ['+', '-', '*', '/']
    #x = [[x[1] for x in f if x[1][1] in ops and x[1][0] == 1] for f in symbol_facts]
    #print(x, flush=True)
...
class Trainer():
  ...
  def train_epoch(self, epoch):
    ...
    for (i, (img_seq, img_seq_len, label, formula)) in enumerate(iter):
      print(i,flush=True)
      block_list = [4,5,6,8,9,10,11]
      if i in block_list:
        continue
      #import code; code.interact(local=locals())
      ...
      #      self.network.eval_formula.scallop_back_time,
      #      back_time - self.network.eval_formula.scallop_back_time,
      #      sep=",",
      #      file=sys.stderr
      #      )
...
if __name__ == "__main__":
  ...
  if args.test_dataset is None:
    args.test_dataset = args.train_dataset.replace("train", "test")

  # Data
  data_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../data"))
  model_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../model/hwf"))
  if not os.path.exists(model_dir): os.makedirs(model_dir)
  train_loader, test_loader = hwf_loader(data_dir, args.train_dataset, args.test_dataset, batch_size=args.batch_size)
