...
class MNISTVideoLSTM(torch.nn.Module):
  def __init__(self, embedding_size=32):
    ...
    self.lstm = torch.nn.LSTM(self.embedding_size, self.embedding_size, batch_first=True)
  ...
  def forward(self, video_batch):
    ...
    frame_embeddings_batch = frame_embeddings_batch_raw.view(batch_size * num_frames, -1)
    digits_batch = self.digit_decoder(frame_embeddings_batch).view(batch_size, num_frames, -1)
    changes_batch = self.changes_decoder(frame_embeddings_batch).view(batch_size, num_frames)
