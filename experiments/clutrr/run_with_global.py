...
class CLUTRRDataset:
  def __init__(self, root, dataset, split):
    ...
    if split == "train":
      self.file_names = [os.path.join(self.dataset_dir, d) for d in ["1.2,1.3_train.csv"]] 
    else:
      self.file_names = [os.path.join(self.dataset_dir, d) for d in ["1.8_test.csv", "1.9_test.csv", "1.10_test.csv"]]
...
def clutrr_loader(root, dataset, batch_size):
  ...
  train_loader = DataLoader(train_dataset, batch_size, collate_fn=CLUTRRDataset.collate_fn, shuffle=False)
...
class CLUTRRModel(nn.Module):
  def __init__(
    ...
    self.scallop_ctx.import_file(os.path.abspath(os.path.join(os.path.abspath(__file__), "../scl/clutrr_gpu.scl")))
    ...
    if self.debug:
      ...
      self.reason = self.scallop_ctx.forward_function("answer", list(range(len(relation_id_map))), dispatch='single')
  ...
  def build_name_maps(self, context_facts, question_facts):
    class NameMap:
      def __init__(self):
        self.storage = {}
      ...
      def get(self, person) -> int:
        if person not in self.storage:
          self.storage[person] = self.people_counter
          self.people_counter += 1
        return self.storage[person]

    batch_size = len(context_facts)
    assert batch_size == len(question_facts)
    batched_name_maps = [NameMap() for i in range(batch_size)]
    context_facts_gpu = [[] for i in range(batch_size)]
    question_facts_gpu = [[] for i in range(batch_size)]
    for (dp_idx, dp_context_facts) in enumerate(context_facts):
      for (p, (rid, name1, name2)) in dp_context_facts:
        id1 = batched_name_maps[dp_idx].get(name1)
        id2 = batched_name_maps[dp_idx].get(name2)
        context_facts_gpu[dp_idx].append((p, (rid, id1, id2)))
    for (dp_idx, dp_question_facts) in enumerate(question_facts):
      for (name1, name2) in dp_question_facts:
        id1 = batched_name_maps[dp_idx].get(name1)
        id2 = batched_name_maps[dp_idx].get(name2)
        question_facts_gpu[dp_idx].append((id1, id2))
    return context_facts_gpu, question_facts_gpu
  ...
  def forward(self, x, phase='train'):
    ...
    context_facts_gpu, question_facts_gpu = self.build_name_maps(context_facts, question_facts)
    result = self.reason(context=context_facts_gpu, question=question_facts_gpu, disjunctions={"context": context_disjunctions})
...
class Trainer:
  ...
  def train(self, num_epochs):
    for i in range(1, num_epochs + 1):
      if args.test:
        ...
        if args.lobster:
          import os
          os.environ["STRATUM"] = "3,3"
          os.environ["CUB"] = "1"
          os.environ["USE_ARENA"] = "1"
          os.environ["ARENA_SIZE"] = "10"
          os.environ["NO_CHECK"] = "1"
          self.test_epoch(i)
          del os.environ["STRATUM"]
          del os.environ["CUB"]
          del os.environ["USE_ARENA"]
          del os.environ["ARENA_SIZE"]
          del os.environ["NO_CHECK"]
        else:
          self.test_epoch(i)
  ...
  def train_epoch(self, epoch):
    ...
    for (i, (x, y)) in enumerate(iterator):
      #block_list = [33, 46,59]
      block_list = [10, 13,34,49,78,221,306]
      if i in block_list:
        continue
